...
async function initDashboard() {
  ...
  const coreOk = q1El && q2El && q3El && errEl && trackEl && pctEl && cardEl;
  if (!coreOk) {
    console.error("Dashboard missing core elements.", {
      dashQ1: !!q1El, dashQ2: !!q2El, dashQ3: !!q3El,
      dashErr: !!errEl, dashProgressTrack: !!trackEl,
      dashProgressPct: !!pctEl, dashProgressCard: !!cardEl
    });
    return;
  }

  // Card click → Learning page
  cardEl.onclick = () => go("/tracker");
  cardEl.onkeydown = (e) => {
    if (e.key === "Enter" || e.key === " ") go("/tracker");
  };

  // Optional quick actions (only wire if present)
  const qaPrimary = document.querySelector("#qaPrimary");
  const qaPrimaryTitle = document.querySelector("#qaPrimaryTitle");
  const qaPrimarySub = document.querySelector("#qaPrimarySub");
  const qaLearn = document.querySelector("#qaLearn");
  const qaInfo = document.querySelector("#qaInfo");
  const qaSettings = document.querySelector("#qaSettings");
  const qaResetOnboarding = document.querySelector("#qaResetOnboarding");
  const qaAskAI = document.querySelector("#qaAskAI");

  // If these are missing, we *don't* crash the page
  if (!qaPrimary || !qaPrimaryTitle || !qaPrimarySub) {
    console.warn("Quick Actions not found in dashboard.html yet (IDs missing).");
  } else {
    // Static routes
    qaLearn && (qaLearn.onclick = () => go("/tracker"));
    qaInfo && (qaInfo.onclick = () => go("/info"));
    qaSettings && (qaSettings.onclick = () => go("/settings"));

    qaResetOnboarding && (qaResetOnboarding.onclick = async () => {
      try {
        const profile = await getProfile();
        await updateProfile({
          ...profile,
    ...
    qaAskAI && (qaAskAI.onclick = () => {
      const btn = document.querySelector("#chatFab") ||
                  document.querySelector(".chat-fab") ||
                  document.querySelector("[data-chat-fab]");
      if (btn) btn.click();
      else alert("AI assistant (placeholder): use the chat bubble bottom-right.");
    });
  }
  ...
  try {
    ...
    // Primary CTA (only if the buttons exist)
    if (qaPrimary && qaPrimaryTitle && qaPrimarySub) {
      const next = LEARNING_QUIZZES.find(q => !completed.includes(q.id));
      if (next) {
        qaPrimaryTitle.textContent = "Start recommended";
        qaPrimarySub.textContent = next.title;
        qaPrimary.onclick = () => go(`/quiz?id=${encodeURIComponent(next.id)}`);
      } else {
        qaPrimaryTitle.textContent = "All complete";
        qaPrimarySub.textContent = "Review a quiz";
        qaPrimary.onclick = () => go(`/quiz?id=${encodeURIComponent(LEARNING_QUIZZES[0].id)}`);
      }
    }
